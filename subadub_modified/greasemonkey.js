...
(function initializeSubadub() {
  ...
  function getSelectedTrackInfo() {
      ...
      const matchingTracks = trackList.filter(
          (el) => el.id === selectedTrackId
      );
      ...
      if (!webvttCache.has(cacheKey)) {
          ...
          fetch(url)
              ...
              .then(function (blob) {
                  webvttCache.set(
                      cacheKey,
                      new Blob([blob], { type: "text/vtt" })
                  ); // set type to avoid warning
                  renderAndReconcile();
              })
  ...
  function downloadSRT() {
      ...
      for (const elem of document.querySelectorAll(".video-title *")) {
          if (!elem.firstElementChild && elem.textContent) {
              // only get 'leaf' elements with text
  ...
  function renderAndReconcile() {
      ...
      function addTrackElem(videoElem, blob, srclang) {
          ...
          if (!playerElem) {
              throw new Error(
                  "Couldn't find player element to append subtitles to"
              );
          }
  ...
  document.body.addEventListener(
      "keydown",
      function (e) {
          if (e.keyCode === 67 && !e.altKey && !e.ctrlKey && !e.metaKey) {
              // unmodified C key
